...
export default function HomeScreen() {
  ...
  const loadDailyChallenge = useCallback((uid: string) => {
    const today = new Date().toISOString().slice(0, 10);
    AsyncStorage.getItem(`@compscicarno_daily_${uid}_${today}`)
      .then(val => {
        // Guard: discard result if the user changed while this request was in flight
        if (uidRef.current !== uid) return;
        if (val !== null) {
        ...
        <View className="px-6 pt-4 pb-6">
          <View className="mb-4">
            ...
            <Text className="text-2xl font-bold text-foreground">
              Ready to explore computer science?
            </Text>
          </View>
            ...
            <View className="bg-primary/5 rounded-2xl p-5 border border-primary/20">
              <View className="flex-row items-start gap-3">
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-primary text-xs mt-2">— CompSciCarno Wisdom</Text>
                </View>
              </View>
            </View>
          </Animated.View>
