...
const userDoc    = (uid: string) => doc(db, 'compscicarno_users', uid);
const progressDoc = (uid: string, worldId: number) =>
  doc(db, 'compscicarno_users', uid, 'worldProgress', String(worldId));
const achievementDoc = (uid: string, achId: string) =>
  doc(db, 'compscicarno_users', uid, 'achievements', achId);
const dailyStatDoc = (uid: string, dateKey: string) =>
  doc(db, 'compscicarno_users', uid, 'dailyStats', dateKey);
const leaderboardDoc = (uid: string) => doc(db, 'compscicarno_leaderboard', uid);
...
export async function ensureUserDoc(uid: string, defaults: { name?: string }): Promise<void> {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      name:               defaults.name ?? 'CompSciCarno Explorer',
      avatarId:           0,
      points:             0,
      level:              1,
      inventory:          [],
...
export async function getWorldProgress(uid: string): Promise<Record<string, WorldProgressDoc>> {
  const colRef = collection(db, 'compscicarno_users', uid, 'worldProgress');
...
export async function getUserAchievements(uid: string): Promise<string[]> {
  const colRef = collection(db, 'compscicarno_users', uid, 'achievements');
  const snap = await getDocs(colRef);
...
export async function getUserDailyStats(uid: string): Promise<Array<{dateKey: string; questionsSolved?: number; pointsEarned?: number; loggedIn?: boolean}>> {
  const colRef = collection(db, 'compscicarno_users', uid, 'dailyStats');
  const snap = await getDocs(colRef);
...
export async function getLeaderboard(limitCount = 50): Promise<Array<{uid: string; displayName: string; points: number; updatedAt?: unknown}>> {
  const q = query(
    collection(db, 'compscicarno_leaderboard'),
    orderBy('points', 'desc'),
    limit(limitCount),
  );
